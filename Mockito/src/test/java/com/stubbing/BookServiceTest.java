...
public class BookServiceTest {
	...
	public void testCalculateTotalCostOfBooks() {
		...
		when(bookRepository.findBookByBookId("1234"))
				.thenReturn(book1)
				.thenReturn(book1);
        when(bookRepository.findBookByBookId("1235")).thenReturn(book2,book2);
		
//        doReturn(book1).when(bookRepository).findBookByBookId("1234");
//        doReturn(book1).when(bookRepository).findBookByBookId("1235");
		
		int actualCost = bookService.calculateTotalCost(bookIds);
		assertEquals(1800, actualCost);
	}
	
	
	// Stubbing with void method
	// In this it actually not checking content is same or not - it only check references of book object in line 54 and inside bookservice line no 35 
	@Test
	...
	// if we not done equeal and hashcode method then it gives PotentialStubbingProblem exception
	// This time it checkes content of books
	@Test
